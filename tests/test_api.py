...
def test_upload_page_uses_employee_friendly_labels(monkeypatch):
    ...
    assert '"max_optimization_seconds": 180' in response.text
    assert "recommended for Railway uploads" in response.text
    assert '"max_orders": 5' in response.text
    assert '"order_summary"' in response.text
...
def test_power_upload_disabled_by_default(monkeypatch):
    monkeypatch.delenv("BOX_OPTIMIZER_ENABLE_POWER_UPLOAD", raising=False)
    monkeypatch.delenv("BOX_OPTIMIZER_UPLOAD_TOKEN", raising=False)
    client = TestClient(app)

    response = client.get("/power-upload")

    assert response.status_code == 404
    assert "Power Upload Disabled" in response.text


def test_power_upload_page_uses_balanced_180_defaults_when_enabled(monkeypatch):
    monkeypatch.setenv("BOX_OPTIMIZER_ENABLE_POWER_UPLOAD", "true")
    monkeypatch.delenv("BOX_OPTIMIZER_UPLOAD_TOKEN", raising=False)
    client = TestClient(app)
    ...
    assert response.status_code == 200
    assert "Box Optimizer Power Upload" in response.text
    assert "Power Balanced Mode is intended for local runs" in response.text
    assert 'action="/power-upload"' in response.text
    assert '&quot;packing_mode&quot;: &quot;balanced&quot;' in response.text
    assert '&quot;max_optimization_seconds&quot;: 180' in response.text


def test_power_upload_workflow_uses_existing_job_download_path(monkeypatch, tmp_path):
    monkeypatch.setenv("BOX_OPTIMIZER_ENABLE_POWER_UPLOAD", "true")
    monkeypatch.delenv("BOX_OPTIMIZER_UPLOAD_TOKEN", raising=False)
    monkeypatch.setenv("BOX_OPTIMIZER_JOBS_DIR", str(tmp_path / "jobs"))
    captured = {}
    ...
    assert "Optimization Results" in response.text
    assert captured["config"]["packing_mode"] == "balanced"
    assert captured["config"]["max_optimization_seconds"] == 180
    job_id = _extract_job_id(response.text)
    download_response = client.get(f"/jobs/{job_id}/download")
    assert download_response.status_code == 200
    assert download_response.content[:2] == b"PK"
